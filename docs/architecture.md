...
Tokens are otherwise stateless, but two additive revocation mechanisms sit on top: every access token carries a random `jti`, and `POST /auth/logout` denylists that single `jti` (in Redis, same instance as the rate limiter, TTL bounded to the token's own remaining lifetime) so a token cannot be used again once its owner explicitly logs out. Separately, `POST /admin/users/{id}/revoke-sessions` lets an administrator invalidate every token already issued to a user -- e.g. responding to a compromised account -- by recording a per-user "tokens issued before this instant are void" cutoff, without needing to know any individual `jti`. Both checks run on every authenticated request (`resolve_admin_user`) and fail open on a Redis outage (falls back to a per-process store, matching the rate limiter's convention) since revocation is additive protection on top of the existing expiry-based JWT, not the primary authentication boundary.
