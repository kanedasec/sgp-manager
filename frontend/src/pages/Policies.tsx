import { Eye, Filter, Plus, ShieldAlert, ShieldOff, Trash2 } from 'lucide-react'
...
export default function Policies() {
  ...
  const remove = async (policy: Policy) => {
    setError('')
    if (!window.confirm(`Permanently delete this bypass policy for "${policy.application_name}"? This cannot be undone.`)) return
    try {
      await api(`/api/v1/admin/bypass-policies/${policy.id}`, { method: 'DELETE' })
      if (selected?.id === policy.id) setSelected(null)
      await load()
    } catch (e) { setError(e instanceof Error ? e.message : 'Could not delete policy') }
  }
